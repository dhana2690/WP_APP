...
$gProgramCode = "ADVERTISINGUNITMAINTENANCE";
...
$dataSource = new DataSource("advertising_units");
...
switch ($urlPage) {
	...
	default:
		...
		$listColumnData[] = array("field_name"=>"image_width","sort_order"=>"image_width","label"=>"Image Width","align"=>"center");
		$listColumnData[] = array("field_name"=>"image_height","sort_order"=>"image_height","label"=>"Image Height","align"=>"center");
		$listColumnData[] = array("field_name"=>"default_rate","sort_order"=>"default_rate","label"=>"Default Rate","align"=>"right","format"=>"currency");
...
<head>
	...
	<link rel="stylesheet" href="tmpl/styles_admin.css">
...
<div class="format_section" style="margin: 80px 200px;">
...
<tr>
	<td class='label'>Image Width</td>
	<td><input type="text" name="image_width" id="image_width" size="12" value="<?php echo htmlspecialchars($dataRow['image_width'],ENT_QUOTES,'UTF-8') ?>" data-crc="<?php echo getCrc32($dataRow['image_width']) ?>" /></td>
</tr>
<tr>
	<td class='label'>Image Height</td>
	<td><input type="text" name="image_height" id="image_width" size="12" value="<?php echo htmlspecialchars($dataRow['image_height'],ENT_QUOTES,'UTF-8') ?>" data-crc="<?php echo getCrc32($dataRow['image_height']) ?>" /></td>
</tr>
<tr>
	<td class='label'>Default Rate</td>
	<td><input type="text" name="default_rate" id="default_rate" size="12" value="<?php echo htmlspecialchars($dataRow['default_rate'],ENT_QUOTES,'UTF-8') ?>" data-crc="<?php echo getCrc32($dataRow['default_rate']) ?>" /></td>
</tr>
<tr>
...
<tr>
	<td></td>
	<td><input type="checkbox" name="internal_use_only" id="internal_use_only" value="Y"<?php echo ($dataRow['internal_use_only'] == 1 ? " checked" : "") ?> /><span  class='label'>&nbsp;Internal Use Only</span></td>
</tr>
<tr>
	<td></td>
	<td><input type="checkbox" name="inactive" id="inactive" value="Y"<?php echo ($dataRow['inactive'] == 1 ? " checked" : "") ?> /><span  class='label'>&nbsp;Inactive</span></td>
</tr>
</table>

<div id="button_div" class="top_border" style="margin: 40px 0 0 0;">
...
<div class="format_section" style="margin: 20px 180px;">
...
<tr>
	...
	<?php } ?>
	<td align="right"><button name="add_button_from_list" id="add_button_from_list" >Add Unit</button></td>
...
<table class="grid-table" id="list_table">
	...
	<?php foreach ($listColumnData as $column) { ?>
		<th class='list<?php echo (empty($column['align'])?'':' ' . $column['align']) ?>'><a class="column-header" data-sort_order="<?php echo $column['sort_order'] ?>"><?php echo $column['label'] ?><?php echo ($dataSource->getSortOrder(true) == $column['sort_order'] ? "&nbsp;" . ($dataSource->getReverseSort() ? "&uarr;" : "&darr;") : "") ?></a></th>
	<?php } ?>
	</tr>
	<?php
	$rowNumber = 0;
	...
	<tr class='list_row'>
	<?php foreach ($listColumnData as $column) { ?>
		<td class='list<?php echo (empty($column['align'])?'':' ' . $column['align']) ?>'><a<?php echo ($rowNumber < 10 ? " accesskey='" . $rowNumber . "'" : "") ?> class="record-link" data-primary_id="<?php echo $dataRow['advertising_unit_id'] ?>"><?php echo htmlspecialchars($dataRow[$column['field_name']],ENT_QUOTES,"UTF-8") ?></a></td>
	<?php } ?>
	</tr>
	<?php } ?>
</table>
...
<div id="empty_list">
	No advertising units.
</div>

<?php } ?>

<div id="button_div">
	<button class="control-button" name="add_button" id="add_button" >Add Unit</button>
</div>
