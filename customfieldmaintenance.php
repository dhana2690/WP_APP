...
$gProgramCode = "CUSTOMFIELDMAINTENANCE";
include_once "scpt/utilities.inc";
include_once "scpt/class.datasource.php";

$gItemsPerPage = 30;

$dealerContactInfo = getDealerContactInfo($gDealerId);

$dataSource = new DataSource("custom_fields");
...
<head>
	<meta charset="utf-8">
	<title><?php echo $dealerContactInfo['dealer_name'] . " | " . $gProgramTitle; ?></title>
...
<tr>
	<td></td>
	<td><input type="checkbox" name="required" id="required" value="Y"<?php echo ($dataRow['required'] == 1 ? " checked" : "") ?> /><span  class='label'><label for="required">&nbsp;Data is required</label></span></td>
</tr>
<tr>
	<td></td>
...
<tr>
	...
	<?php } ?>
	<td align="right"><button name="add_button_from_list" id="add_button_from_list" >Add Custom Field</button></td>
...
<table class="grid-table" id="list_table">
	...
	<tr class='list_row'>
	<?php foreach ($listColumnData as $column) { ?>
		<td class='list'><a<?php echo ($rowNumber < 10 ? " accesskey='" . $rowNumber . "'" : "") ?> class="record-link" data-primary_id="<?php echo $dataRow['custom_field_id'] ?>"><?php echo htmlspecialchars($dataRow[$column['field_name']],ENT_QUOTES,"UTF-8") ?></a></td>
	<?php } ?>
	</tr>
	<?php } ?>
</table>
...
<div id="button_div">
	<button class="control-button" name="add_button" id="add_button" >Add Custom Field</button>
</div>
