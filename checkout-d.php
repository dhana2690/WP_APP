...
<html lang="en">
    <head>
        ...
        <title><?php
echo ($metaTagArray['istitleinchild'] == '1' ? $metaTagArray['title'] : $metaTagArray['title']);
if (empty($metaTagArray['title'])) {
   echo $dealerArray['dealer_name'];
}
?></title>
        <meta name="description" content=<?php
        echo($metaTagArray['isdescriptioninchild'] == '1' ? "Checkout - " . $metaTagArray['description'] : $metaTagArray['description']);
...
<?php include_once "scpt/google_code.inc"; ?>
    ...
    <body>
        ...
        <?php
        $preferenceCode = getFieldFromId('preference_id', 'preferences', 'preference_code', 'STICKY_INTERACT');
        $enableStickyInteract = getFieldFromId('preference_value', 'dealer_preferences', 'dealer_id', 1, 'preference_id = ' . $preferenceCode);

        if ($enableStickyInteract == 0) {
           ?>
           <!--Script is created for Test Instance -->
                     <!-- <script src="//interact.juststicky.com/js/embed/client.js/2116/interact_5841e556a4cec" id="interact_5841e556a4cec" data-text="Discuss this with Sticky Interact" data-unique="5841e556a4cec"></script>  -->
           <!--Script is created for DEV Environment -->
           <script src="//istage.juststicky.com/js/embed/client.js/2116/interact_58b71df490aa3" id="interact_58b71df490aa3" data-text="Discuss this with Sticky Interact" data-unique="58b71df490aa3"></script>
...
<?php 
      $ApruvdCatListquery  = "select * from categories where dealer_id is null and department_id in (6,7,8,11) and inactive = 0 and internal_use_only = 0 ";
      $categoryResult = executeQuery($ApruvdCatListquery);
      $catList= array();
      if ($categoryResult['row_count'] > 0) {	
         while ($categoryRow = getNextRow($categoryResult)) {
            array_push($catList,$categoryRow['category_id']);
         }
      }
?>
                        <input type="hidden" id="appruvd_catlist" value="<?php echo implode(',',$catList) ?>">
...
<?php if ($checkoutDisabled === true || $shoppingCartDisabled === true) { ?>
   ...
   <?php } ?>
                               ...
                               <div id="contact_info">
                                   ...
                                   <table cellpadding="0" cellspacing="3" width="250px"><tr>
                                           <td valign="top">
                ...
                <!--<td><input type="text" id="email_address" name="email_address" class="field info" style="width: 160px;" value="<?php //echo $_POST['email'];   ?>"></td>-->
                                                   ...
                                                   <tr>
                                                       <td width="150" align="right" class="label" style="white-space:nowrap;overflow: hidden;padding:10px 0 3px 0; ">Confirm Email:&nbsp;</td>
                                                       <td><input type="text" id="confirm_email_address" name="confirm_email_address" class="field info" style="width: 160px;" value="<?php echo $_SESSION['cart_info']['email_address']; ?>"><td>
                                                   </tr>
   ...
   <?php } ?>
                                           ...
                                           <td>
                                               ...
                                               <textarea class="field info" id="notes" name="notes" style="width: 241px; height: 65px;"><?php echo $_SESSION['cart_info']['notes']; ?></textarea>
                                           </td>
                                       </tr></table>
                               </div>
...
<?php include_once (empty($templateArray['footer']) ? "templates/default" : $templateArray['footer']) . "/footer.inc"; ?>
        ...
        <?php
        $resultSet = executeQuery("select * from subscription where manufacturer_id='205' and inactive=0");
        if ($row = getNextRow($resultSet)) {
           $statusId = $row['status'];
        }
        ?>
