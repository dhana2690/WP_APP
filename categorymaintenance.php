...
$gProgramCode = "CATEGORYMAINTENANCE";
...
switch ($urlAction) {
	case "save":
		if (empty($_POST['link_name'])) {
			$originalLinkName = $_POST['description'];
		} else {
			$originalLinkName = $_POST['link_name'];
		}
		// build the link_name
		$linkName = trim(preg_replace('/\s+/', '-',trim(str_replace("-"," ",$originalLinkName))),"-");
		$linkName = preg_replace('/[^a-zA-Z0-9\-]/','',$linkName);
		while (strpos($linkName,"--") !== false) {
			$linkName = str_replace('--', '-',$linkName);
		}
		$linkName = trim(strtolower($linkName),"-");
		if ($gUserTypeId == 2) {
			$linkName = trim($linkName,"-" . $gDealerId) . "-" . $gDealerId;
		}
		if ($linkName != $originalLinkName) {
			// make sure link_name is unique
			while (true) {
				$resultSet = executeQuery("select * from categories where link_name = ?",$linkName);
				if ($resultSet['row_count'] == 0) {
					break;
				}
				if ($gUserTypeId == 2) {
					$linkName = trim($linkName,"-" . $gDealerId) . "-" . rand(1,100) . "-" . $gDealerId;
				} else {
					$linkName .= rand(1,100);
				}
			}
		}
		$_POST['link_name'] = $linkName;
...
switch ($urlPage) {
	...
	default:
		...
		$listColumnData[] = array("field_name"=>"department_name","sort_order"=>"department_name","label"=>"Department","align"=>"center");
		$listColumnData[] = array("field_name"=>"description","sort_order"=>"description","label"=>"Category","align"=>"left");
		$listColumnData[] = array("field_name"=>"internal_use_only","sort_order"=>"internal_use_only","label"=>"Hidden","align"=>"center");
		$listColumnData[] = array("field_name"=>"inactive","sort_order"=>"inactive","label"=>"Not Used","align"=>"center");
...
<tr>
	<td class='label'><label for="link_name">Link Name</label></td>
	<td><input type="text" name="link_name" id="link_name" size="40" value="<?php echo htmlspecialchars($dataRow['link_name'],ENT_QUOTES,'UTF-8') ?>" data-crc="<?php echo getCrc32($dataRow['link_name']) ?>" /></td>
</tr>

<tr><td>&nbsp;</td><td></td></tr>

<tr>
	<td class='label'><label for="department_id">Department</label></td>
	<td>
		<select name='department_id' id='department_id' onchange='updateDepartment()'>
		<option value=''>Select...</option>
		<?php
			$resultSet = executeQuery("select * from departments where inactive = 0 and (dealer_id is null or dealer_id = ?) order by department_id",$gDealerId);
			while ($row = getNextRow($resultSet)) {
				echo "<option value='" . $row['department_id'] . "'" . ($row['department_id']==$dataRow['department_id']?" selected":"") . ">" . $row['description'] . "</option>";
			}
		?>
		</select>
	</td>
...
<tr>
	<td class='label'><label for="product_type_id">Product Type</label></td>
	<td>
		<select name='product_type_id' id='product_type_id'>
		<option value=''></option>
		<?php
			$resultSet = executeQuery("select * from product_types where inactive = 0 order by description");
			while ($row = getNextRow($resultSet)) {
				echo "<option value='" . $row['product_type_id'] . "'" . ($row['product_type_id']==$dataRow['product_type_id']?" selected":"") . ">" . $row['description'] . "</option>";
			}
		?>
		</select>
		<span "label">&nbsp;<i>(used to assign shipping group)</i></span>
	</td>
</tr>
...
<tr>
	<td></td>
	<td><input type="checkbox" name="internal_use_only" id="internal_use_only" value="Y"<?php echo ($dataRow['internal_use_only'] == 1 ? " checked" : "") ?> /><span  class='label'><label for="internal_use_only">&nbsp;Internal Use Only <i>(hide from public view)</i></label></span></td>
</tr>
<tr>
	<td></td>
	<td><input type="checkbox" name="inactive" id="inactive" value="Y"<?php echo ($dataRow['inactive'] == 1 ? " checked" : "") ?> /><span  class='label'><label for="inactive">&nbsp;Inactive <i>(no longer used by system)</i></label></span></td>
</tr>
</table>
...
<div id="button_div" class="top_border">
	...
	<?php 
		$resultSet = executeQuery("select * from products where category_id = ?",$dataSource->getPrimaryId());
		if ($resultSet['row_count'] == 0) { ?>
...
<tr>
	...
	<?php } ?>
	<td align="right"><button name="add_button_from_list" id="add_button_from_list" >Add Category</button></td>
...
<table class="grid-table" id="list_table">
	...
	<?php foreach ($listColumnData as $column) { ?>
		<th class='list <?php echo $column['align'] ?>'><a class="column-header" data-sort_order="<?php echo $column['sort_order'] ?>"><?php echo $column['label'] ?><?php echo ($dataSource->getSortOrder(true) == $column['sort_order'] ? "&nbsp;" . ($dataSource->getReverseSort() ? "&uarr;" : "&darr;") : "") ?></a></th>
	<?php } ?>
	</tr>
	<?php
	$rowNumber = 0;
	...
	<tr class='list_row'>
	<?php foreach ($listColumnData as $column) { ?>
		<td class='list <?php echo $column['align'] ?>'><a<?php echo ($rowNumber < 10 ? " accesskey='" . $rowNumber . "'" : "") ?> class="record-link" data-primary_id="<?php echo $dataRow['category_id'] ?>">
			<?php 
			if (is_numeric($dataRow[$column['field_name']])) {
				echo ($dataRow[$column['field_name']] == 1 ? "&radic;" : "");
			} else {
...
<div id="button_div">
	<button class="control-button" name="add_button" id="add_button" >Add Category</button>
</div>
...
function verifyFields() {
	...
	if (!error && ($("#description").val() == "" || $("#description").val().length < 1)) {
		error = showError($("#description"),"Category Name cannot be blank");
	}
